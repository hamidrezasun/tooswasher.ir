...
@router.post("/", response_model=page_schemas.Page)
def create_page(
...
@router.get("/", response_model=List[page_schemas.Page])
def read_pages(
    skip: int = 0,
    limit: int = 100,
    db: Session = Depends(get_db)
...
@router.get("/{page_id}", response_model=page_schemas.Page)
def read_page(
    page_id: int,
    db: Session = Depends(get_db)
):
...
@router.put("/{page_id}", response_model=page_schemas.Page)
def update_page(
    page_id: int,
